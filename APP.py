...
DEFAULT_DB = {
    "KLAVUU": [
        ...
        "KLAVUU White Pearlsation Ideal Actress Backstage Cream Spf30 Pa++Lavende",
    ...
    "TIRTIR": [
        ...
        "TIRTIR MASK FIT ALL-COVER CUSHION 13N FAIR IVORY",
        "TIRTIR MASK FIT ALL-COVER CUSHION 15C FAIR PORCELAIN",
        "TIRTIR MASK FIT ALL-COVER CUSHION 17C PORCELAIN",
        "TIRTIR MASK FIT ALL-COVER CUSHION 17W FRENCH VANILLA",
        "TIRTIR MASK FIT ALL-COVER CUSHION 21C COOL IVORY",
        "TIRTIR MASK FIT ALL-COVER CUSHION 21N IVORY",
        "TIRTIR MASK FIT ALL-COVER CUSHION 21W NATURAL IVORY",
        "TIRTIR MASK FIT ALL-COVER CUSHION 23N SAND",
        "TIRTIR MASK FIT ALL-COVER CUSHION 24N LATTE",
        "TIRTIR MASK FIT ALL-COVER CUSHION 24W SOFT BEIGE",
        "TIRTIR MASK FIT ALL-COVER CUSHION 25N MOCHA",
        "TIRTIR MASK FIT ALL-COVER CUSHION 27C COOL BEIGE",
        "TIRTIR MASK FIT ALL-COVER CUSHION 27N CAMEL",
        "TIRTIR MASK FIT ALL-COVER CUSHION 29N NATURAL BEIGE",
    ],
    "K-SECRET": [
...
if "OFFICIAL_DB" not in st.session_state:
    st.session_state["OFFICIAL_DB"] = DEFAULT_DB

db_dict = st.session_state["OFFICIAL_DB"]


def get_all_official_names():
    return [name for sublist in db_dict.values() for name in sublist]
...
def find_best_match(invoice_name):
    all_names = get_all_official_names()

    if invoice_name in all_names:
        return invoice_name, "100% 완벽 일치", "PASS"

    norm_inv = normalize_text(invoice_name)

    for official in all_names:
        if norm_inv == normalize_text(official):
            return official, "대소문자/띄어쓰기/특수문자 차이", "WARNING"
...
# ---------------------------------------------------------
# [2] 사이드바 - DB 현황 및 TXT 다운로드 & DB 추가 기능
# ---------------------------------------------------------
st.sidebar.header("📚 등록된 브랜드 DB 현황")
st.sidebar.caption(
    "브랜드 옆 [📄 TXT] 버튼을 클릭하면 제품목록을 받아볼 수 있습니다."
)

for brand, items in list(db_dict.items()):
...
new_product_name = st.sidebar.text_input(
    "추가할 정식 제품명 입력",
    placeholder="예: TIRTIR CERAMIC MILK AMPOULE",
)
...
st.write(
    "업로드한 INVOICE 엑셀의 제품명이 베트남 공표확인서 정식 제품명과 100% 일치하는지 정밀 검수합니다."
)
...
uploaded_file = st.file_uploader(
    "📂 검수할 INVOICE 엑셀 파일(.xlsx)을 업로드하세요", type=["xlsx"]
)
...
if uploaded_file is not None:
    try:
        ...
        for idx in range(len(df_raw)):
            row = df_raw.iloc[idx]
            no_val = str(row[0]).strip() if pd.notna(row[0]) else ""
            desc_val = str(row[1]).strip() if pd.notna(row[1]) else ""

            if "TOTAL AMOUNT" in no_val or "TOTAL AMOUNT" in desc_val:
                break

            if no_val.isdigit() and desc_val and desc_val != "nan":
                unit_size = str(row[2]).strip() if pd.notna(row[2]) else ""
                qty = str(row[3]).strip() if pd.notna(row[3]) else ""

                items_list.append(
                    {
                        "No": int(no_val),
                        "Invoice_Name": desc_val,
                        "Unit_Size": unit_size,
                        "Qty": qty,
                    }
                )
        ...
        if not items_list:
            st.error(
                "❌ 인보이스에서 제품 목록을 찾지 못했습니다. 엑셀 서식을 확인해 주세요."
            )
        else:
            results = []
            ...
            st.dataframe(
                df_result.style.map(highlight_status, subset=["검수 상태"]),
            ...
            st.download_button(
                ...
                data=excel_data,
                file_name="RIESTI_INVOICE_검수결과.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
